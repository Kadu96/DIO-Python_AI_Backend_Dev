...
class ContaCorrente(Conta):
    ...
    def __repr__(self) -> str:
        return f"<{self.__class__.__name__}: ('{self.agencia}', '{self.nro_conta}','{self.cliente.nome}')>"
...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        if conta.historico.transacoes_do_dia() <= 10:
            ...
            print("\n ##\tAtingido o Número Máximo de Transações permitidas para o dia!!!\t##\n")
...
class Historico:
    ...
    def transacoes_do_dia(self):
        ...
        for transacao in self._transacoes:
            data_transacao = datetime.strptime(transacao["data"], "%d/%m/%Y %H:%M:%S").strftime("%d/%m/%Y")
...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        log = f"{reg_transacao},{func.__name__.upper()}, Argumentos: {args}{kwargs},{resultado}\n"
        if os.path.exists(ROOT_PATH / "log.txt"):
            try:
                with open(ROOT_PATH / "log.txt", "a", newline="", encoding="utf-8") as arquivo_log:
                    arquivo_log.writelines(log)
            except IOError as exc:
                print(f"Erro ao carregar o arquivo!!\n{exc}")
        else:
            try:
                with open(ROOT_PATH / "log.txt", "w", encoding="utf-8") as arquivo_log:
