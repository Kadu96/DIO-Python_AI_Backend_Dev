...
shutil.move(
    ROOT_PATH / "arquivo_teste.txt",
    ROOT_PATH / "arquivos_teste" / "arquivo_teste.txt"
)  # altera o dir de um arquivo

os.rename(
    ROOT_PATH / "arquivos_teste" / "arquivo_teste.txt",
    ROOT_PATH / "arquivos_teste" / "arquivo_novo_01.txt",
)  # renomeia um arquivo
...
try:
    with open(
        ROOT_PATH / "arquivos_teste" / "novo_exemplo.txt", "w", encoding="utf-8"
    ) as arquivo:
...
try:
    with open(
        ROOT_PATH / "arquivos_teste" / "novo_exemplo.txt", "r", encoding="ascii"
    ) as arquivo:
        print(arquivo.read())
except IOError as exc:
    print(f"Erro ao abrir o arquivo!!\n{exc}")
